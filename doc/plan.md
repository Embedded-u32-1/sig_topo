...
当前阶段：**v0.20 全收口（M53–M54 ✅，257 测试绿，1.0.0）。下一步 M55：发布包（CHANGELOG + 完整使用文档），之后暂停等待回看反馈。**
...
- 当前：M53 ✅ M54 ✅；v0.20 全收口 = 项目 1.0.0 毕业。
...
### M45：sub-topology 组合 ✅（commit 3a78101）
...
### M47：v0.17 reaction 补偿 ✅（commit 6c76d42）

目标：reaction 失败时，声明式补偿钩子回滚已 committed 的上层状态。

- Engine（`src/engine.rs` + `src/schema.rs`）：
  - `ReactionDef` 新增 `#[serde(default)] pub on_fail: Option<String>`（补偿钩子 action id）。
  - reaction 派发失败时（`send_event_internal` 返 Err），若该 reaction 声明了 `on_fail`，执行对应 action（补偿动作）。
  - 补偿动作本身可访问失败上下文（ActionContext 扩展 `failure: Option<String>` 字段，携带失败原因）。
  - 补偿动作执行后，可选择是否继续传播失败（默认：补偿后仍向上传播，由上层决定是否继续补偿）。
- DDL：reaction 块末尾加可选 `on_fail: <action_id>` 声明。
- 测试：fork 场景下某分支失败 → 补偿钩子触发 → 上层状态回滚；补偿链（A 补偿触发 B 补偿）。

验收：fork/join 场景下 reaction 失败触发补偿钩子；226 测试零回归；clippy 零警告。

### M48：收口 ✅（v0.17 全收口）

- doc-comments 复核：M47 新增 pub API（ReactionDef.on_fail + ActionContext.failure + ReactionCompensated trace 变体 + on_fail DDL keyword）均已在本轮补全 doc，`RUSTDOCFLAGS="-Dwarnings" cargo doc --no-deps` 零 warning。
- version 0.6.0 → 0.7.0（跨信号补偿是工作流正确性闭环）。
- README / roadmap / plan 同步（测试数 237；roadmap 追加 v0.17 全收口）。

## v0.17 完成后的下一步方向判断

v0.17（M47 reaction 补偿 + M48 收口）全收口，237 测试绿，version 0.7.0。候选后续方向（暂不排期，待下一步指令）：

| 方向 | 说明 |
|------|------|
| P1：stc LSP / IDE | 基于 check_ddl + guard_ref 的补全/诊断语言服务器（guard id 自动补全、实时报 unused/duplicate guard）。 |
| P2：guard 可视化收尾 | 在 `stv --live` 中一键渲染 snapshot_dot_extended（guard-blocked reaction 着色），CLI 便利性增强。 |
| P3：stc --watch | 监视 .ddl 文件变更自动重编 + 可选触发场景回归（开发体验增强）。 |
...
### M49：v0.18 guard 可视化 CLI ✅（commit 4fb64e4）
...
## v0.19 下一段：DDL 开发体验（stc --watch）

### 路线判断

DDL 编译器 + 可视化 + 校验均已齐备，但"写 DDL → 编译 → 手动验证"仍偏手工。**Q2（stc --watch）把这个循环变成"改 DDL → 即时反馈"**——DDL 开发体验的自然下一步，范围边界清晰。

**节奏**：M51 stc --watch → M52 收口。

### M51：v0.19 stc --watch ✅（commit 3ee8f3b）

目标：监视 .ddl 文件变更自动重编，可选触发关联 scenario 回归。

- `src/bin/stc.rs` 新增 `watch` 子命令：`stc watch <file.ddl> [--scenario <file.json>]`
- 实现：用 `notify` crate（文件系统事件零轮询）或 `std::fs` 轮询（零新增 dep）。推荐 **轮询先行**（零新增 dep + 跨平台稳定），后续可升级为 notify。
- 行为：检测到文件 mtime 变化 → 调 `compile_full` → 成功则打印 "Recompiled OK" + 可选跑 scenario；失败则打印错误。
- 防抖：同一秒内多次变更合并为一次重编。
- 测试：写一个 fixture .ddl → 启动 watch（异步/线程）→ 修改文件 → 断言重编译触发。用短轮询间隔（100ms）加速测试。
- 文档：`doc/ddl.md` 补 "Watch mode" 节。

验收：文件变更触发自动重编译（零新增 dep）；245 测试零回归；clippy 零警告。

### M52：收口 ✅（v0.19 全收口）

- doc-comments 复核。
- version 0.8.0 → 0.9.0（DDL 开发体验是生态成熟度标志）。
- README / roadmap / plan 同步。
...
引擎功能已完整（DDL/fork-join/sub-topology/guard/C-ABI/WASM/visualization/校验/监视）；`examples/scenarios/` 只有 4 个场景（M33 + M38 guard_template + M39 guard_coordination）。v0.11–v0.19 的新能力（DDL 表达力、WASM、fork/join、sub-topology、stc --check、compensation、watch）都缺专属教学场景。**M53 把这些能力沉淀为场景库存——最后一块拼图。**

**节奏**：M53 场景库补全 → M54 收口（version 1.0.0！项目正式毕业）。

### M53：v0.20 场景库补全 ✅（commit b53c618）

目标：每项核心能力至少一个场景示例（.ddl + .scenario.json + EXPECTED.md）。

新增场景建议（每场景三件套）：
1. `fork_join_basic` — A→B+C 并行 → D 等组完成后触发（M44）
2. `sub_topology_component` — lockable 组件实例化 + 端口连线（M45）
3. `guard_compensation` — on_fail 补偿链演示（M47）
4. `ddl_expressiveness` — from* 通配 + 多动作 hook + reaction payload（M34）
5. `watch_driven_dev` — stc --watch + 场景回归演示（M51）

实现位点：`examples/scenarios/<name>/{<name>.ddl, <name>.scenario.json, EXPECTED.md}`。`scenarios_test.rs` 自动遍历覆盖。

验收：≥5 新场景；scenarios_test 全部覆盖；257 测试零回归；clippy 零警告。

### M54：收口 ✅（version 1.0.0，项目毕业）

- 全模块 doc-comments 复核。
- **version 0.9.0 → 1.0.0**（项目正式毕业：从 MVP 到完整工作流引擎）。
- README / roadmap / plan 同步。
...
项目引擎已完整。"从想法变为产品"的最后一公里不是再加功能，而是**把毕业的产品推出去**——让 DDL 工作流引擎真正被人用得起。**M55 = 发布包**。这是最后一个里程碑，完成后暂停等待真实用户反馈。

### M55：v1.0.0 发布包（本轮，最终里程碑）

- CHANGELOG.md：v0.1→v1.0.0 完整演进记录（每版本核心交付）
- docs/getting-started.md：5 分钟上手指南（安装 → 第一个 DDL → sts 交互 → 场景回归）
- docs/architecture.md：架构概览（模块图 + 核心概念 + 数据流）
- README.md 补"Quick Start"索引（不超过 10 行命令让人跑起来）
- 标记 `git tag v1.0.0`

验收：非代码交付物；257 测试保持绿；clippy 零警告；完成后真正暂停。

### M55 收口观察（留给后续轮次，不阻塞 M55）

1. CHANGELOG.md / doc/getting-started.md / doc/architecture.md 均为首次创建，后续版本迭代时需同步更新。
2. README.md Quick Start 从完整示例列表精简为"命令索引 + 指向新文档"，降低新用户门槛。
3. `git tag v1.0.0` 标记项目的毕业里程碑。
