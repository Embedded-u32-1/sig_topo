...
当前阶段：**v0.13 全收口（M34–M37 ✅，177 测试绿，0.3.0）。空闲决策点后，经判断启动 v0.14 新方向：reaction guard 跨信号协调语义深化。M38 起由 agent 逐步实现。**
...
### M38：v0.14 reaction guard 复合语义 —— 设计 + 实现（本轮起）
...
### M39：guard demo 场景 + stc --check 增强
...
- 全模块 doc-comments 复核（M38 触及的 trace/engine/parser/codegen）。
- version 0.3.0 → 0.4.0（跨信号协调是质的飞跃）。
