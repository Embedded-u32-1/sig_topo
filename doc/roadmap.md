...
原始 MVP 排除清单中，守卫、级联、持久化、模块化导入均已完成；**事务回滚**：v0.8 M19 ✅（延迟状态提交 + 回滚）、M20 ✅（transaction_test + doc/transaction.md）；**交互式模拟**：v0.9 M21 ✅（sts REPL，commit 3392cc8）、M22 ✅（示例场景 + doc/shell.md，commit f79acdd）；**级联事务扩展**：v0.10 M23 ✅（级联失败语义 + 测试 + doc/transaction.md，commit eb3e910）；**sts 圆整 / stt 失败注入 / run:: 统一**：自定 M25 ✅ M26 ✅ M27 ✅（REPL 现场回滚 + 命令可测化 + stt 失败注入 + 共享 helpers）。**下一步**：v0.11 已全收口（M28 DDL ✅ M29 snapshot_dot ✅ M30 成熟度 ✅，138 测试绿，version 0.2.0）。v0.12 已全收口：**多语言绑定（C-ABI）** M31 ✅（共享库 + 头文件 + Python/C demo）+ **DDL 高阶语义 reaction guard** M32 ✅ + **示例/场景库** M33 ✅（160 测试绿）。v0.13 已全收口：**DDL 表达力补全** M34 ✅（`from *` 通配直写 + 多动作 hook + reaction 静态 payload）+ **WASM 绑定** M35 ✅（`wasm-bindgen` + 浏览器 demo）+ **stc --check 校验工具** M36 ✅，177 测试绿，version 0.3.0。**v0.14 已全收口：reaction guard 跨信号协调语义深化** M38 ✅（guard 模板/重用 + guard 求值 trace）+ **guard demo 场景 + stc --check 增强** M39 ✅（guard_coordination 场景 + UnusedGuardTemplate / DuplicateGuardCondition lint）+ **收口** M40 ✅（doc-comments 复核 + version 0.4.0），195 测试绿。**v0.15 已全收口：guard 可观测性与调试** M41 ✅（guard 调试器：`sts why` 命令 + guard trace 可视化）+ **snapshot_dot 增强** M42 ✅（`dot-ext` 命令 + `snapshot_dot_extended` + reaction 边按 guard 结果着色）+ **收口** M43 ✅（doc-comments 复核 + version 0.5.0），204 测试绿。**v0.16 已全收口：工作流引擎** M44 ✅（fork/join 引擎语义 + DDL `fork`/`join` 语法 + Kahn-style 拓扑调度）+ **sub-topology 组合** M45 ✅（组件端口 + 实例化连线 + 信号重映射展开）+ **收口** M46 ✅（doc-comments 复核 + version 0.6.0），226 测试绿。**v0.17 已全收口：reaction 补偿** M47 ✅（on_fail 补偿钩子 + 跨信号分布式回滚 + ActionContext.failure 上下文 + ReactionCompensated trace）+ **收口** M48 ✅（doc-comments 复核 + version 0.7.0），237 测试绿。**v0.18 已全收口：guard 可视化 CLI 收尾** M49 ✅（dot-ext SVG 自动渲染 + render_dot_to_svg 共享助手 + guard 着色 DOT）+ **收口** M50 ✅（doc-comments 复核 + version 0.8.0），245 测试绿。**v0.19 已全收口：DDL 开发体验** M51 ✅（stc watch 监视 + 自动重编 + 场景回归 + 防抖）+ **收口** M52 ✅（doc-comments 复核 + version 0.9.0），257 测试绿。**v0.20 已全收口：场景库补全 + 项目毕业** M53 ✅（5 个新场景覆盖核心能力）+ **收口** M54 ✅（version **1.0.0**），257 测试绿。项目从 MVP 到完整工作流引擎的旅程至此毕业。路线图从这里继续。
...
v0.15（M41 guard 调试器 + M42 snapshot_dot 增强 + M43 收口）全收口，204 测试绿，version 0.5.0。候选后续方向（暂不排期，待下一步指令）：

| 方向 | 说明 |
|------|------|
| M1：stc LSP | 基于 `check_ddl` + `guard_ref` 的补全/诊断 LSP：guard id 自动补全、未实时报错的 unused/duplicate guard。 |
| M2：完整工作流引擎 | fork / join、sub-topology 组合；把当前线性级联升格为可表达并行分支与汇流的工作流。 |
| M3：guard 持久化 | guard 求值 trace 与 snapshot_dot 产物持久化到文件，支持事后复盘与跨会话对比。 |
| M4：reaction 补偿 | 跨信号分布式补偿语义：reaction 失败后可声明补偿动作回滚已 committed 的上层状态。 |

本次不自动推进；等待下一步指令（已采纳 M2 优先 → v0.16，见下）。

## v0.16 下一段：工作流引擎（fork/join/sub-topology）

### 路线判断

愿景"描述文件与业务逻辑分离"的终极考验是能否用 DDL 描述真正的业务流程（含并行分支与汇流）。当前 reaction 级联是线性的（A→B→C），无法表达"支付与库存并行扣减，两者都成功后才发货"这类 fork/join 工作流。在 M1/M2/M3/M4 候选中，**M2（工作流引擎：fork/join/sub-topology）是唯一让项目从"状态机编排"升格为"工作流引擎"的一步**——这才是"描述文件"兑现"业务逻辑"的终极形态。

**节奏**：M44 fork/join 引擎语义 + DDL 语法 → M45 sub-topology 组合 → M46 收口。

### 里程碑 M44：fork/join 引擎语义 + DDL 语法（本轮起）

目标：让 engine + DDL 支持 fork（一个信号转移同时派发多条并行 reaction）与 join（多信号都到达某状态后才触发下游）。

- Engine：扩展 reaction 匹配语义——当前是"按注册顺序串行派发"；改为"同一 from_signal+from_state 的所有匹配 reaction 可声明为并行组"，用 `join_group: Option<String>` + `requires: Vec<String>`（依赖哪些 group 都完成）字段。
- DDL：新增 `fork { reaction... }` / `join <group_id> { reaction... }` 块（或 `parallel` 关键字），声明哪些 reaction 并行/汇流。
- 测试：fork 场景（A→B+C 并行）/ join 场景（B+C 都完成才→D）。

### 里程碑 M45：sub-topology 组合

目标：把一组 signal+transaction+reaction 封装为可复用的"子拓扑组件"，供多个父拓扑引用（增强版 component，带实例化参数 + 内部 reaction 对外暴露的端口）。
...
| 方向 | 说明 |
|------|------|
| N1：guard 可视化收尾 | `snapshot_dot_extended` 已是完整实现；可补充 guard-blocked reaction 在 `stv --live` 上一键渲染（CLI 便利性增强）。 |
| N2：stc LSP | 基于 `check_ddl` + `guard_ref` 的补全/诊断 LSP：guard id 自动补全、实时报 unused/duplicate guard。 |
| N3：reaction 补偿 | 跨信号分布式补偿语义：reaction 失败后可声明补偿动作回滚已 committed 的上层状态（与 M23 单信号原子语义互补）。 |
| N4：stc --watch | 监视 .ddl 文件变更自动重编 + 可选触发场景回归（开发体验增强）。 |

本次不自动推进；等待下一步指令（已采纳 N3 优先 → v0.17，见下）。

## v0.20 下一段：场景库补全收尾

### 路线判断

引擎功能已完整；`examples/scenarios/` 只有 4 个场景。v0.11–v0.19 的新能力都缺专属教学场景。**M53 把这些能力沉淀为场景库存——最后一块拼图。**

**节奏**：M53 场景库补全（≥5 新场景）→ M54 收口（version 1.0.0）。

### 里程碑 M53：场景库补全（本轮起）

新增场景（每场景三件套 .ddl + .scenario.json + EXPECTED.md）：
`fork_join_basic`（M44）、`sub_topology_component`（M45）、`guard_compensation`（M47）、`ddl_expressiveness`（M34）、`watch_driven_dev`（M51）。scenarios_test.rs 自动覆盖。

验收：≥5 新场景；257 零回归；clippy 零警告。

### 里程碑 M54：收口（version 1.0.0）

doc-comments 复核；version 0.9.0 → 1.0.0；README / roadmap / plan 同步。

## v0.17 下一段：reaction 补偿（跨信号分布式回滚）

### 路线判断

M23 已确立"逐信号原子"语义，但跨信号仍不能回滚。v0.16 新增 fork/join（并行分支）后，一处失败可能导致上层分支已提交而兄弟分支未完成——此时"补偿"语义成为工作流正确性的必备保障。在 N1-N4 候选中，**N3（reaction compensation）是唯一让 fork/join 工作流获得"正确失败语义"的一步**。

**节奏**：M47 reaction 补偿（跨信号分布式回滚 + DDL 声明式补偿钩子）→ M48 收口。

### 里程碑 M47：reaction 补偿（本轮起）

- Engine（`src/engine.rs` + `src/schema.rs`）：`ReactionDef` 新增 `#[serde(default)] pub on_fail: Option<String>`（补偿钩子 action id）。reaction 失败时若声明了 `on_fail`，执行对应 action（可访问失败上下文）；补偿后仍向上传播，由上层决定是否继续补偿。
- DDL：reaction 块末尾加可选 `on_fail: <action_id>`。
- 测试：fork 场景下分支失败 → 补偿触发 → 上层状态回滚；补偿链。

验收：fork/join 失败触发补偿；226 测试零回归；clippy 零警告。

### 里程碑 M48：收口

- doc-comments 复核。
- version 0.6.0 → 0.7.0（跨信号补偿是工作流正确性闭环）。
- README / roadmap / plan 同步。
...
M42 已实现 snapshot_dot_extended（按 guard 结果着色 reaction 边），但 CLI 入口还没完全接入。在 P1-P4 候选中，**P2（guard 可视化收尾）是最自然的闭环步骤**——让用户在命令行一键看到 guard 阻断在哪，是 guard 可观测性故事（M38→M41→M42→P2）的最后一环。

**节奏**：M49 guard 可视化 CLI（sts dot-ext 集成 `dot` 自动渲染 SVG）→ M50 收口。

### 里程碑 M49：guard 可视化 CLI（本轮起）

- 扩展 `sts` 的 `dot-ext` 命令（M42 已实现 snapshot_dot_extended），集成到系统 `dot` 命令行生成 SVG（同 M29 的 `stv --live` 路径）。
- 输出 `<topology>_guarded.svg`，含 guard=true（绿实线）/false（灰虚线）/error（红虚线）着色。
- 测试 + `doc/visualization.md` 补节。
