...
- **v0.7 模块化导入**（M16 ✅）：`ComponentDef`/`InstanceDef`、`expand()` 参数化组件展开、`src/compose.rs`

> M16 code review 发现 2 个 Major 正确性问题，下一增量落地时一并修复：
> - M1：`subst` 用 `HashMap` 迭代导致跨运行非确定 + 二次扫描已替换文本 → 改为从左到右单次扫描。
> - M2：`MissingBinding` 错误里组件名写成字面量 `<component>` → 透传真实组件名。

原始 MVP 排除清单中，守卫、级联、持久化均已完成；**尚未完成的结构性项目**：模块化导入/组件复用（M17–M18 进行中）、交互式模拟调试、并发安全/事务回滚、多语言绑定。路线图从这里继续。
...
## 后续方向（概要，待 v0.7 完成后展开为里程碑）

以下在原 MVP 排除清单（事务回滚、模拟调试、并发/多语言绑定）中，按「描述文件驱动」这一核心理念的优先级排列：

- **v0.8 事务与回滚**：当前若 `on_enter` 动作在状态已翻转后失败，系统处于不一致状态。改为「延迟提交 + 失败回滚」：先校验守卫、再运行全部生命周期动作，全部成功后才提交 `StateChanged`；任一失败则状态不变并返回错误。这是当前真实存在的正确性问题。
- **v0.9 交互式模拟**：`sts` 单步步入 REPL + 实时 trace，让整条链路「描述文件驱动」对用户可感知、可调试。
- **v0.10 多语言绑定**：C-ABI / WASM，让非 Rust 业务代码也能驱动引擎，真正兑现「描述文件 vs 业务逻辑分离」。
